...
import logo150 from "../assets/crc-peridona-logo.png";
...
export default function HomePageHeader() {
  ...
  return (
    <>
      ...
      <header className="w-full bg-white border-b border-[rgba(196,154,43,0.2)] shadow-sm fixed left-0 right-0 top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 flex items-stretch gap-0 h-[72px]">
                ...
                <Link
                  ...
                  className={`relative text-[13px] font-bold px-3 py-1.5 whitespace-nowrap tracking-widest uppercase transition-colors
                    ${isActive(item.href)
                      ...
                      : "text-[#555] hover:text-[#C49A2B] after:absolute after:bottom-0 after:left-3 after:right-3 after:h-[2px] after:bg-[#C49A2B] after:rounded-full after:origin-left after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300"
                    }`}
                >
                  {item.label}
                </Link>
          ...
          <div className="hidden xl:flex items-center pl-6 border-l border-[rgba(196,154,43,0.2)] flex-shrink-0">
            <Link
              href="/contact-us"
              className="btn-shine btn-gold-glow hover-lift animate-pulse-glow group inline-flex items-center gap-2 bg-[#C49A2B] hover:bg-[#A8841E] text-white text-[10px] font-semibold uppercase tracking-widest px-5 py-2.5 transition-colors"
            >
              Enquire Now
          ...
          <button
            ...
            className="xl:hidden ml-auto flex items-center justify-center w-10 h-10 rounded flex-shrink-0 transition-all duration-200 active:scale-90 self-center"
      ...
      {/* Mobile drawer */}
      <div
        className={`fixed top-0 left-0 h-screen z-50 flex flex-col shadow-2xl transition-transform duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] xl:hidden ${
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(196,154,43,0.2)]">
          <Image src={logo150} alt="CRC The Peridona" style={{ width: "90px", height: "auto" }} sizes="90px" />
          <button
            type="button"
            onClick={() => setOpen(false)}
            aria-label="Close menu"
            className="flex items-center justify-center w-9 h-9 rounded-full text-[#888] hover:text-[#C49A2B] hover:bg-[rgba(196,154,43,0.08)] text-xl transition-all duration-300 hover:rotate-90 active:scale-90"
          >
            ✕
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto flex flex-col font-sans">
          {siteNavLinks.map((item, i) => (
            <Link
              ...
              href={item.href}
              onClick={() => setOpen(false)}
              className={`${open ? "drawer-item" : "opacity-0"} group relative px-6 py-3.5 border-b border-[rgba(196,154,43,0.1)] text-[15px] font-bold tracking-wide uppercase transition-colors active:bg-[rgba(196,154,43,0.08)] ${
                isActive(item.href)
                  ? "text-[#C49A2B] bg-[rgba(196,154,43,0.05)] border-l-[3px] border-l-[#C49A2B]"
                  : "text-[#444] hover:text-[#C49A2B] hover:pl-7"
              }`}
              style={{ "--i": i }}
            >
              {item.label}
        ...
        <div className="p-5 border-t border-[rgba(196,154,43,0.2)]">
          <Link
            href="/contact-us"
            onClick={() => setOpen(false)}
            className="btn-shine flex justify-center items-center gap-2 py-3 bg-[#C49A2B] hover:bg-[#A8841E] text-white text-sm font-semibold uppercase tracking-widest transition-colors active:scale-[0.98]"
          >
            Enquire Now
          </Link>
        </div>
      ...
      {open && (
        <button
          ...
          className="animate-overlay-fade fixed inset-0 bg-black/40 backdrop-blur-[2px] z-40 xl:hidden"
          onClick={() => setOpen(false)}
        />
      )}
    </>
